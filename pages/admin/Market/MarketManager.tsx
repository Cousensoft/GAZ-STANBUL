...
import { ShoppingBag, BarChart3, Users, Package, ShoppingCart, DollarSign, RotateCcw } from 'lucide-react';
import MarketOverview from './MarketOverview';
import MarketVendors from './MarketVendors';
import MarketProducts from './MarketProducts';
import MarketOrders from './MarketOrders';
import MarketFinance from './MarketFinance';
import MarketReturns from './MarketReturns';

const MarketManager = () => {
    const [activeTab, setActiveTab] = useState<'dashboard' | 'vendors' | 'products' | 'orders' | 'finance' | 'returns'>('dashboard');
    ...
    const renderContent = () => {
        switch(activeTab) {
            ...
            case 'returns': return <MarketReturns />;
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-2 rounded-2xl border border-slate-200 shadow-sm gap-4">
                <div className="flex items-center gap-3 px-4 py-2">
                    <div className="p-2 bg-blue-50 text-blue-600 rounded-xl"><ShoppingBag size={24} /></div>
                    <div><h2 className="text-lg font-bold text-slate-900">GazMarket Yönetimi</h2><p className="text-xs text-slate-500">Süper Admin Paneli</p></div>
                </div>
                <div className="flex overflow-x-auto pb-2 md:pb-0 gap-1 px-2 w-full md:w-auto">
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}><tab.icon size={16} />{tab.label}</button>
                    ))}
                </div>
            </div>
