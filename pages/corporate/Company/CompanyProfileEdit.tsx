...
import { Save, Briefcase, Image as ImageIcon, MapPin, Clock, FileText, Camera, Upload, PlusCircle, X, Navigation, Crosshair, Info, ShieldCheck, CheckCircle, AlertCircle, Trash2, File } from 'lucide-react';
...
const CompanyProfileEdit = () => {
   ...
   // State for Documents (4 Zorunlu Belge)
   ...
   // Check if verified (Yetki Belgesi yüklü mü?)
   const isVerified = documents.find(d => d.id === 'auth_cert')?.uploaded;

   const handleUpload = (id: string) => {
       // Simulate upload
       const date = new Date().toLocaleDateString('tr-TR');
       setDocuments(prev => prev.map(doc => 
           doc.id === id ? { ...doc, uploaded: true, file: 'sample_file.pdf', date } : doc
       ));
   };
      ...
      <div className="animate-fade-in max-w-6xl pb-10">
         <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
                ...
                {isVerified && (
                    <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-black uppercase flex items-center gap-1 border border-blue-200 animate-fade-in">
                        <CheckCircle size={14} fill="currentColor" className="text-blue-500" /> Doğrulanmış Firma
                    </span>
                )}
            </div>
            <button className="bg-slate-900 text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-slate-800 transition-colors flex items-center gap-2 shadow-lg shadow-slate-200">
               <Save size={16} /> Kaydet
            </button>
         </div>
                     ...
                     <div className="animate-fade-in">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                           <div>
                              ...
                              <input type="text" defaultValue="Bosphorus Enerji Ltd. Şti." className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm font-bold text-slate-900 outline-none focus:ring-2 focus:ring-slate-900 transition-all" />
                           </div>
                           <div>
                              <label className="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">Firma Yetkilisi</label>
                              <input type="text" defaultValue="Ahmet Yılmaz" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-slate-900 transition-all" />
                           </div>
                           <div>
                              <label className="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">Firma E-Mail</label>
                              <input type="email" defaultValue="info@bosphorusenerji.com" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-slate-900 transition-all" />
                           </div>
                           <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                           <div>
                              ...
                              <input type="tel" defaultValue="0212 555 00 00" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-slate-900 transition-all" />
                           </div>
                           <div>
                              <label className="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">GSM Tel</label>
                              <input type="tel" defaultValue="0532 555 00 00" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-slate-900 transition-all" />
                           </div>
                           <div>
                              <label className="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">Fax Tel</label>
                              <input type="tel" defaultValue="0212 555 00 01" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-slate-900 transition-all" />
                           </div>
                        </div>
                        ...
                        <div className="flex justify-between items-start mb-8 border-b border-slate-100 pb-6">
                            <div>
                                ...
                                <p className="text-sm text-slate-500 mt-2 max-w-2xl">
                                    Firmanızın onaylanması ve güvenilirlik rozeti alabilmesi için <b>Yetki Belgesi</b> zorunludur. Diğer belgeler kurumsal puanınızı artırır.
                                </p>
                            </div>
                            <div className="bg-blue-50 px-4 py-2 rounded-xl border border-blue-100 flex items-center gap-3">
                                <Info size={20} className="text-blue-600"/>
                                <div className="text-xs text-blue-800">
                                    <span className="font-bold block uppercase tracking-wide">DOĞRULAMA DURUMU</span>
                                    {isVerified ? (
                                        <span className="text-green-600 font-bold flex items-center gap-1 mt-0.5">Onaylandı <CheckCircle size={14}/></span>
                                    ) : (
                                        <span className="text-red-600 font-bold flex items-center gap-1 mt-0.5">Onaylanmadı <AlertCircle size={14}/></span>
                                    )}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-10">
                           <div className="flex flex-col md:flex-row gap-10">
                              ...
                              <div className="flex-1">
                                 ...
                                 <div className="w-full h-32 bg-slate-100 rounded-2xl border-2 border-dashed border-slate-300 flex items-center justify-center relative group overflow-hidden cursor-pointer">
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity flex-col gap-2">
                                       ...
                                       <span className="text-xs font-bold text-slate-600">Arkaplan Değiştir</span>
                                    </div>
                                 </div>
                              </div>
                           </div>
                     ...
                     <div className="animate-fade-in">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                           ...
                           <div>
                              ...
                              <input type="text" defaultValue="Moda" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-slate-900 transition-all" />
                           </div>
                           <div>
                              <label className="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">Mahalle</label>
                              <input type="text" defaultValue="Caferağa Mah." className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-slate-900 transition-all" />
                           </div>
                        </div>
                        ...
                        <div>
                           ...
                           <p className="text-xs text-slate-500 mt-3 flex items-center gap-2 font-medium">
                               <Info size={14} className="text-blue-500"/> Haritadaki konum, müşterilerin "Yakınımdaki Ustalar" aramasında çıkmanızı sağlar.
                           </p>
                        </div>
                     </div>
                  )}
